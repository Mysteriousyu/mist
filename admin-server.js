...
function resolveTargets(assistant, messages) {
  ...
  return chain
    ...
    .map(t => {
      ...
      return p && p.apiKey ? { providerCfg: p, model: t.model } : null;
    })
    .filter(Boolean);
}
...
async function handleChat(req, res) {
  ...
  const assistant = body.assistant === 'mist2' ? 'mist2' : 'mist1';
...
async function handleAdminApi(req, res, urlPath) {
  ...
  if (urlPath === '/admin/state' && req.method === 'GET') {
    ...
    for (const [id, p] of Object.entries(DB.providers))
      providers[id] = { label: p.label, format: p.format, baseUrl: p.baseUrl || '', hasKey: !!p.apiKey, keyMask: mask(p.apiKey) };
    return send(res, 200, {
      providers, routing: DB.routing,
      systemPrompts: DB.systemPrompts || { mist1: '', mist2: '' },
...
function addChainRow(chainId){
  ...
  div.innerHTML='<div><select id="'+chainId+'-p-'+i+'">'+provOpts(Object.keys(STATE.providers)[0])+'</select></div>'+
    ...
    '<button class="btn sm danger" style="flex:none;margin-top:6px" onclick="this.closest(\'.row\').remove()">✕</button>';
...
function renderKeys(){
  ...
  for(const [id,p] of Object.entries(P)){
    html+='<div class="prov"><h4>'+esc(p.label)+' <span class="muted">('+esc(id)+' · '+esc(p.format)+')</span> '+
      (p.hasKey?'<span class="badge ok">key set '+esc(p.keyMask)+'</span>':'<span class="badge no">no key</span>')+'</h4>'+
      '<div class="row">'+
