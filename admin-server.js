...
function renderKeys(){
  ...
  var html='<div class="card"><h3 style="margin-top:0">API providers</h3>'
    +'<p class="muted">Keys stay on this server. Paste to set/replace; leave blank to keep.</p>';
  Object.keys(P).forEach(function(id){
    ...
    html+='<div class="prov"><h4>'+esc(p.label)+' <span class="muted">('+esc(id)+' '+esc(p.format)+')</span> '
      ...
      +'</h4>'
      +'<div class="row"><div><label>Label</label><input value="'+esc(p.label)+'" id="pl-'+id+'"></div>'
      +'<div><label>Format</label><select id="pf-'+id+'"><option'+(p.format==='openai'?' selected':'')+'>openai</option><option'+(p.format==='anthropic'?' selected':'')+'>anthropic</option><option'+(p.format==='gemini'?' selected':'')+'>gemini</option></select></div></div>'
      +'<div class="field"><label>Base URL</label><input value="'+esc(p.baseUrl)+'" id="pb-'+id+'" placeholder="https://..."></div>'
      +'<div class="field"><label>API key</label><input type="password" id="pk-'+id+'" placeholder="'+(p.hasKey?'leave blank to keep':'paste key')+'"></div>'
      +'<button class="btn sm grad" data-save-prov="'+id+'">Save</button> '
      +'<button class="btn sm danger" data-del-prov="'+id+'">Remove</button></div>';
  });
  html+='<button class="btn sm" id="addProvBtn">+ Add provider</button></div>';
  ...
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">Mist 1 routing</h3>'
    ...
    +'<div><label>Model</label><input id="m1-m" value="'+esc(r1.model||'')+'" placeholder="google/gemma-3-27b-it"></div></div>'
    +'<button class="btn grad" style="margin-top:12px" id="saveM1">Save Mist 1</button></div>';

  /* Mist 2 */
  var r2=R.mist2||{};
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">Mist 2 routing (up to 6 per chain)</h3>'
    +'<h4>Coding chain</h4><div id="m2c-rows">'+chainRows('m2c',r2.codingChain)+'</div><button class="btn sm" data-add-row="m2c">+ Add</button>'
    +'<h4 style="margin-top:12px">Multimodal chain</h4><div id="m2m-rows">'+chainRows('m2m',r2.multimodalChain)+'</div><button class="btn sm" data-add-row="m2m">+ Add</button>'
    +'<h4 style="margin-top:12px">Fallbacks</h4><div id="m2f-rows">'+chainRows('m2f',r2.fallbacks)+'</div><button class="btn sm" data-add-row="m2f">+ Add</button>'
    +'<button class="btn grad" style="margin-top:14px" id="saveM2">Save Mist 2</button></div>';

  /* Stella */
  var sa=STATE.stellaAssistant||'mist2';
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">Stella (Stellar AI)</h3>'
  ...
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">Shared Google account</h3>'
    +'<p class="muted">Link one Google account for both Mist AIs to read Gmail/Calendar via API.</p>'
  ...
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">System prompts</h3>'
    +'<p class="muted">Override built-in personality. Leave blank for defaults.</p>'
    +'<h4>Mist 1</h4><textarea id="sp-mist1">'+esc(SP.mist1||'')+'</textarea>'
    ...
    +'<h4 style="margin-top:12px">Mist 2</h4><textarea id="sp-mist2">'+esc(SP.mist2||'')+'</textarea>'
    +'<div style="display:flex;gap:8px;margin-top:6px"><button class="btn sm grad" data-save-sp="mist2">Save</button><button class="btn sm danger" data-clear-sp="mist2">Reset</button></div></div>';
...
document.addEventListener('click',function(e){
  var t=e.target,el;
  /* providers */
  el=t.closest('[data-save-prov]');if(el){var id=el.dataset.saveProv;api('/provider',{id:id,label:$('#pl-'+id).value,format:$('#pf-'+id).value,baseUrl:$('#pb-'+id).value,apiKey:$('#pk-'+id).value}).then(function(){toast('Saved');loadState();});return;}
