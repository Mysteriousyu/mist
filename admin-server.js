...
async function handleChat(req, res) {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    let upstream;
    try {
      // 30 second timeout — if provider doesn't respond, skip to next
      init.signal = AbortSignal.timeout(30000);
      upstream = await fetch(url, init);
    } catch (e) {
      console.error('Provider ' + (providerCfg.label || 'unknown') + ' (' + model + ') failed:', e.message);
      if (i === targets.length - 1) return send(res, 502, { error: 'All providers timed out or failed. Try again.' });
      continue;
    }
    if (!upstream.ok || !upstream.body) {
      const txt = await upstream.text().catch(() => '');
      console.error('Provider ' + (providerCfg.label || 'unknown') + ' (' + model + ') returned ' + upstream.status + ':', txt.slice(0, 200));
      if (i < targets.length - 1) continue;
      return send(res, upstream.status, { error: 'Provider error: ' + txt.slice(0, 300) });
    }
