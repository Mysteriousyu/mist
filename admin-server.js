...
function chainEditor(chainId, chain){
  const rows = (chain||[]).map((f,i)=>
    ...
    '<div><input id="'+chainId+'-m-'+i+'" value="'+esc(f.model||'')+'" placeholder="model id e.g. grok-4-latest"></div>'+
    '<button class="btn sm danger" style="flex:none;margin-top:6px" data-rm-chain data-chain="'+chainId+'" data-idx="'+i+'">✕</button></div>').join('');
  return '<div id="'+chainId+'-rows">'+rows+'</div>'+
    '<button class="btn sm" style="margin-top:8px" data-add-chain="'+chainId+'">+ Add model (max 6)</button>';
}
...
function addChainRow(chainId){
  ...
  div.innerHTML='<div><select id="'+chainId+'-p-'+i+'">'+provOpts(Object.keys(STATE.providers)[0])+'</select></div>'+
    ...
    '<button class="btn sm danger" style="flex:none;margin-top:6px" data-rm-chain data-chain="'+chainId+'" data-idx="'+i+'">✕</button>';
...
// Event delegation so chain add/remove never needs inline onclick strings
document.addEventListener('click', function(e){
  // chain add/remove
  const add = e.target.closest('[data-add-chain]');
  if(add){ addChainRow(add.dataset.addChain); return; }
  const rm = e.target.closest('[data-rm-chain]');
  if(rm){ removeChainRow(rm.dataset.chain, rm.dataset.idx); return; }
  // provider save/delete
  const saveProv = e.target.closest('[data-save-prov]');
  if(saveProv){ saveProvider(saveProv.dataset.saveProv); return; }
  const delProv = e.target.closest('[data-del-prov]');
  if(delProv){ delProvider(delProv.dataset.delProv); return; }
  // prompts
  const savePr = e.target.closest('[data-save-prompt]');
  if(savePr){ savePrompt(savePr.dataset.savePrompt); return; }
  const clearPr = e.target.closest('[data-clear-prompt]');
  if(clearPr){ clearPrompt(clearPr.dataset.clearPrompt); return; }
  // users
  const banBtn = e.target.closest('[data-ban]');
  if(banBtn){ ban(banBtn.dataset.ban, banBtn.dataset.banned === 'true'); return; }
  const delU = e.target.closest('[data-del-user]');
  if(delU){ delUser(delU.dataset.delUser); return; }
  // chats
  const viewC = e.target.closest('[data-view-chat]');
  if(viewC){ viewChat(viewC.dataset.viewChat); return; }
  const delC = e.target.closest('[data-del-chat]');
  if(delC){ delChat(delC.dataset.delChat); return; }
  // projects
  const delP = e.target.closest('[data-del-proj]');
  if(delP){ delProject(delP.dataset.delProj); return; }
});
// File inputs for prompt upload (change event, not click)
document.addEventListener('change', function(e){
  const lp = e.target.closest('[data-load-prompt]');
  if(lp && e.target.files && e.target.files[0]) { loadPromptFile(e.target, lp.dataset.loadPrompt); }
});
...
function renderKeys(){
  ...
  for(const [id,p] of Object.entries(P)){
    html+='<div class="prov"><h4>'+esc(p.label)+' <span class="muted">('+esc(id)+' · '+esc(p.format)+')</span> '+
      ...
      '<button class="btn sm grad" data-save-prov="'+id+'">Save</button> '+
      '<button class="btn sm danger" data-del-prov="'+id+'">Remove</button></div>';
  }
  ...
  html+='<div class="card" style="margin-top:14px"><h3 style="margin-top:0">System prompts</h3>'+
    ...
    '<label class="btn sm" style="cursor:pointer">Upload .txt <input type="file" accept=".txt" style="display:none" data-load-prompt="mist1"></label>'+
    '<button class="btn sm grad" data-save-prompt="mist1">Save Mist 1 prompt</button>'+
    '<button class="btn sm danger" data-clear-prompt="mist1">Reset to default</button></div>'+
    ...
    '<div style="display:flex;gap:8px;margin-top:6px">'+
    '<label class="btn sm" style="cursor:pointer">Upload .txt <input type="file" accept=".txt" style="display:none" data-load-prompt="mist2"></label>'+
    '<button class="btn sm grad" data-save-prompt="mist2">Save Mist 2 prompt</button>'+
    '<button class="btn sm danger" data-clear-prompt="mist2">Reset to default</button></div></div>';
...
function renderUsers(){
  const rows = STATE.users.map(u=>'<tr><td>'+esc(u.id)+'</td>'+
    ...
    '<td style="text-align:right"><button class="btn sm" data-ban="'+esc(u.id)+'" data-banned="'+(!u.banned)+'">'+(u.banned?'Unban':'Ban')+'</button> '+
    '<button class="btn sm danger" data-del-user="'+esc(u.id)+'">Delete</button></td></tr>').join('');
...
function renderChats(){
  const rows = STATE.chats.map(c=>'<tr><td>'+esc(c.title||'Untitled')+'</td>'+
    ...
    '<button class="btn sm danger" data-del-chat="'+c.id+'">Delete</button></td></tr>').join('');
...
function renderProjects(){
  const rows = STATE.projects.map(p=>'<tr><td>'+esc(p.name)+'</td><td>'+esc((p.userId||'').slice(0,12))+'</td>'+
    ...
    '<td style="text-align:right"><button class="btn sm danger" data-del-proj="'+p.id+'">Delete</button></td></tr>').join('');
